      ...
      if (upload.ok) {
        ...
        if (results.transcript.length > 0) {
          const prompt = `Please give feedback on the following interview question: ${question} -  given the following transcript: ${
            results.transcript
          } also Please highlight the candidates strengths and potential weaknesses based on their answers and give constructive feedback on how they could make their in person interview more successful and get the job.`
  ...
  useEffect(() => {
    ...
    if (storedQuestions) {
      try {
        ...
        console.log('First Question Video URL:', parsedQuestions[0]?.videoUrl);
        setSelectedInterviewerQuestions(parsedQuestions);
      } catch (error) {
...
console.log("selectedInterviewerQuestions",selectedInterviewerQuestions);
...
>
  ...
  crossOrigin="anonymous"
  src={
    typeof selectedInterviewerQuestions[0]?.videoUrl === 'string' 
          ...
          <div className="w-full h-[40vh] md:w-1/2 md:h-screen bg-[#F1F2F4] relative overflow-hidden">
            <figure
              ...
              style={{
                ...
                backgroundImage: "url('/admin panel.png')",
                boxShadow:
